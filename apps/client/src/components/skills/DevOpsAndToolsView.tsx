...
export default function DevOpsAndToolsView({ data }: DevOpsAndToolsViewProps) {
  ...
  const pipelineStages = useMemo(
    () => [
      {
        stage: '01. Ingress',
        title: 'Git Trigger & Lint',
        detail: 'Pre-commit hooks, TypeScript checks & linting',
        duration: '12s',
        icon: GitBranch,
      },
      {
        ...
        detail: 'Unit tests, E2E validation & coverage threshold',
        duration: '26s',
        icon: ShieldCheck,
      },
      {
        ...
        detail: `${skillName} multi-stage compilation & caching`,
        duration: '42s',
        icon: Package,
      },
      {
        ...
        detail: 'Dependency audit & container security verification',
        duration: '14s',
        icon: Lock,
      },
      {
        ...
        detail: 'Zero-downtime cluster update & health check',
        duration: '18s',
        icon: Rocket,
      },
      {
        ...
        detail: 'Prometheus metrics, Grafana & uptime logging',
        duration: 'Active',
        icon: Activity,
      },
    ],
  ...
  return (
    <motion.div
      ...
      animate="visible"
    >
      {/* PAGE TITLE & DEVOPS RUNTIME TELEMETRY BAR */}
      <motion.div
      ...
      {/* TOP ROW: INTRO CARD, PROFICIENCY GAUGE, INFRASTRUCTURE TENETS */}
      ...
      {/* CI/CD DEPLOYMENT PIPELINE LIFECYCLE */}
      <motion.div
        variants={cardVariants}
      ...
      {/* MIDDLE ROW: PRODUCTION ENVIRONMENTS, INFRASTRUCTURE BLUEPRINTS, IMPACT & STACK */}
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {/* Production Applications & Deployments */}
        <motion.div
          variants={cardVariants}
          whileHover={{ y: -3 }}
          className="rounded-[12px] border border-border/40 bg-card p-6 shadow-sm transition-shadow duration-300 hover:shadow-md"
        ...
        {/* What I Build (Infrastructure Assets) */}
        <motion.div
          variants={cardVariants}
          whileHover={{ y: -3 }}
          className="rounded-[12px] border border-border/40 bg-card p-6 shadow-sm transition-shadow duration-300 hover:shadow-md"
        ...
        {/* Impact & Performance Metrics + Tech Stack */}
        <motion.div
          variants={cardVariants}
          whileHover={{ y: -3 }}
          className="rounded-[12px] border border-border/40 bg-card p-6 shadow-sm transition-shadow duration-300 hover:shadow-md"

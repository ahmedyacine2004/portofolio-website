...
export function StatusBar() {
  ...
  const { isPlaying, togglePlayback, hydrate } = useAudioStore();
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="hidden lg:flex h-full w-full items-center justify-between rounded-xs bg-background px-3 py-1 shadow-gray-400 dark:shadow-[0_0_5px_rgba(255,255,255,0.015)]"
      >
        {/* ================================================================
            LEFT SIDE
        ...
        <div className="absolute left-1/2 -translate-x-1/2 text-[10px] text-foreground-secondary">
      ...
      {/* TABLET STATUS BAR - Reduced content */}
      <motion.div
        initial={{ opacity: 0, y: 24 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{
        ...
        className="hidden md:flex lg:hidden h-full w-full items-center justify-between rounded-xs bg-background px-2 py-0.5 shadow-gray-400 dark:shadow-[0_0_5px_rgba(255,255,255,0.015)]"
      >
        {/* Left - Current page */}
        <div className="flex shrink-0 items-center gap-1 rounded-xs bg-background-secondary px-1.5 py-0.5 text-[8px] leading-none text-brand">
          <CurrentIcon className="size-2.5" strokeWidth={1.8} aria-hidden="true" />
          <span>{currentPageLabel}</span>
        </div>

        {/* Center - Time */}
        <span className="text-[8px] text-foreground-secondary">{time}</span>

        {/* Right - Theme and status */}
        <div className="flex shrink-0 items-center gap-1">
          {mounted ? (
            <button
              type="button"
              onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
              className="flex items-center text-[8px] leading-none transition-colors hover:text-foreground"
              aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} theme`}
            >
              {theme === 'dark' ? (
                <Sun className="size-2.5" strokeWidth={1.8} aria-hidden="true" />
              ) : (
                <Moon className="size-2.5" strokeWidth={1.8} aria-hidden="true" />
              )}
            </button>
          ) : null}

          <span className="flex items-center gap-0.5 text-[8px] font-medium text-foreground">
      ...
      {/* MOBILE STATUS BAR - Hidden on phone screens */}
